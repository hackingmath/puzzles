...
https://twitter.com/1to9puzzle/status/1184879042928898048
Numoku variation with gray squares having to be
either larger or smaller than all adjacent squares
October 17, 2019"""
...
BOARD = '.......6..1..............1..5.......'
grays = [0,2,3,5,10,11,13,16,17,19,21,25,27,33,34]
...
def nbs(board,n):
    """Returns list of all neighbors of square n"""
    r = n // 6
    c = n % 6

    neighbs = []
    if r > 0:
        neighbs.append(board[n - 6])
    if c > 0:
        neighbs.append(board[n - 1])
    if r < 5:
        neighbs.append(board[n + 6])
    if c < 5:
        neighbs.append(board[n + 1])
...
# board = [random.choice(range(1,10)) for i in range(BOARD.count('.'))]
# board2 = populate_board(board)
# print_board(board2)
# print(max_min_adjacent(board2,2))
